...
| 10 | Q editable en `CREATED`/`SENT`/`ANSWERED` (campos, productos y other charges) — **exige open-lock del usuario**; bloqueada en `COMPLETE`/`REJECTED` | Nuevos errores 400; el front debe abrir la Q antes de editar |
...
### 2.11 Editabilidad de la Q en CREATED / SENT / ANSWERED + open-lock obligatorio

**Endpoints afectados:**

| Método | Ruta | Operación |
|--------|------|-----------|
| `POST` | `/ip/q/{id_quotation}/product` | Add products |
| `PUT` | `/ip/q/{id_quotation}/product/{id_q_product}` | Edit products |
| `DELETE` | `/ip/q/{id_quotation}/product/{id_q_product}` | Remove products |
| `POST` | `/ip/q/{id_quotation}/other_charges` | Add other charge |
| `PUT` | `/ip/q/{id_quotation}/other_charges/{id_other_charge}` | Edit other charge |
| `DELETE` | `/ip/q/{id_quotation}/other_charges/{id_other_charge}` | Remove other charge |
| `POST` | `/ip/q/{id_quotation}/other_charges/import-from-qr` | Import other charges desde QR |
| `DELETE` | `/ip/q/{id_quotation}/other_charges/imported-from-qr/{id}` | Remove imported other charge |
| `PUT` | `/ip/q/{id_quotation}` | Update campos |

**Nueva regla:** Todas estas operaciones ahora exigen **dos condiciones**:

1. **Status editable** — `CREATED`, `SENT` o `ANSWERED`. En `COMPLETE`/`REJECTED` → HTTP 400 con `ip.q.not-editable-by-status`.
2. **Open-lock del usuario** — la Q debe estar abierta por el usuario autenticado (`PATCH /ip/q/open-lock/{id}?type=EDIT`). Si nadie la tiene abierta → 400 `ip.q.not-block`; si la tiene otro usuario → 400 `ip.q.not-block-by` con el nombre de quien la bloquea.

**Mensajes de error (HTTP 400):**

| Key | Mensaje |
|-----|---------|
| `ip.q.not-editable-by-status` | "The Quotation cannot be modified because it is in REJECTED or COMPLETED status" |
| `ip.q.not-block` | "The Quotation is not blocked" |
| `ip.q.not-block-by` | "The Quotation is blocked by {0}" |

**Excepciones / sin cambios:**
- Add/remove de **QR** a la Q: sigue **solo en `CREATED`** (`ip.q.qr.cannot-add` / `ip.q.qr.cannot-delete`).
- `Currency` y `Client`: se mantiene la lógica actual (Client bloqueado si la Q tiene QR asociadas — `ip.q.client-change-blocked`; Currency solo si las QR asociadas tienen otra moneda — `ip.q.currency-mismatch`).
- `change-status` de la Q: **no** exige open-lock.
- Consultas (`GET`), imprimir y clonar: igual que antes.

**Cierre del open-lock:** al pasar la Q a `COMPLETE`/`REJECTED` el backend limpia `openBy` y `openAt` (igual que QR): la Q queda cerrada, deja `GET /ip/q/load-open` y nunca más vuelve a bloquearse.

**Acción Frontend:** **abrir la Q antes de editar** con `PATCH /ip/q/open-lock/{id}?type=EDIT` (campos, productos o other charges). Deshabilitar/ocultar la edición cuando `status ∈ {COMPLETE, REJECTED}` y manejar los 400 `ip.q.not-editable-by-status`, `ip.q.not-block`, `ip.q.not-block-by` por si otro usuario tiene abierta la Q.
...
ip.q.not-created-status       → Reemplazada por ip.q.not-editable-by-status
```
...
| `PATCH /ip/q/{id}/change-status` | **Cambio de respuesta**: ahora devuelve `MessageResponse<IpQuotationResponse>` (objeto completo, igual que `reject`); antes devolvía el resumen `ListIpQuotationResponse` |
...
| `IpQuotationResponse` / `IpQuotationDTO` → `openBy`, `openAt` | Al pasar a `COMPLETE`/`REJECTED` pasan a `null` (open-lock se cierra; la Q no vuelve a bloquearse) |
| Edición de Q (PUT, productos, other charges manuales/importados) | **Cambio de comportamiento**: ahora exige open-lock del usuario y status no terminal (nuevos 400 `ip.q.not-editable-by-status`, `ip.q.not-block`, `ip.q.not-block-by`) |
...
| 6 | Edición de campos de la Q (`PUT /ip/q/{id}`) | Solo en `CREATED`/`SENT`/`ANSWERED` **y con open-lock del usuario** → 400 `ip.q.not-editable-by-status` / `ip.q.not-block` / `ip.q.not-block-by` |
| 7 | Products y Other Charges (add/edit/remove) + import desde QR | Sé permitido en `CREATED`/`SENT`/`ANSWERED`; exige open-lock del usuario (400 `ip.q.not-editable-by-status` / `ip.q.not-block` / `ip.q.not-block-by`) |
| 8 | `Currency` / `Client` en `PUT /ip/q/{id}` | No editables con la regla actual: Client bloqueado si la Q tiene QR asociadas (`ip.q.client-change-blocked`); Currency solo si las QR asociadas tienen otra moneda (`ip.q.currency-mismatch`) |
| 9 | Q pasa a `COMPLETE`/`REJECTED` | Limpia `openBy`/`openAt` (la Q queda cerrada y nunca más se bloquea) |

> **Open-lock:** antes de editar campos/productos/other charges, el front debe abrir la Q con `PATCH /ip/q/open-lock/{id}?type=EDIT`. `change-status` **no** exige open-lock. La fecha de aplicación es **solo fecha** (sin hora) y se valida antes de pasar a `SENT`/`ANSWERED`.
