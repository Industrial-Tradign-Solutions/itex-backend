...
@Getter
@Setter
public class FilterListPurchaseOrder extends BaseFilter<PurchaseOrderEntity> {
    ...
    public Specification<PurchaseOrderEntity> filter() {
    ...
    private Specification<PurchaseOrderEntity> hasNumber() {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.equal(root.get("number"), getNumber());
    }

    private Specification<PurchaseOrderEntity> hasClientId() {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.equal(root.get("client").get("id"), getClientId());
    }

    private Specification<PurchaseOrderEntity> hasSupplierId() {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.equal(root.get("supplier").get("id"), getSupplierId());
    }

    private Specification<PurchaseOrderEntity> hasRemarks() {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.like(criteriaBuilder.lower(root.get("remarks")), "%" + getRemarks().toLowerCase() + "%");
    }

    private Specification<PurchaseOrderEntity> hasStatus() {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.equal(root.get("status"), getStatus());
    }

    private Specification<PurchaseOrderEntity> hasSalesRepId() {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.equal(root.get("salesRep").get("id"), getSalesRepId());
    }

    private Specification<PurchaseOrderEntity> hasProductFilter() {
