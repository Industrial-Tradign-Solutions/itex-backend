...
public interface IIpQuoteRequestProductRepository extends JpaRepository<IpQuoteRequestProductEntity, UUID> {
    @Query("SELECT p FROM IpQuoteRequestProductEntity p WHERE p.id = ?1 AND p.ipQuoteRequest.id = ?2")
    Optional<IpQuoteRequestProductEntity> fetchOneById(UUID id, UUID qrId);

    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN TRUE ELSE FALSE END FROM IpQuoteRequestProductEntity p WHERE p.ipProduct.id = ?1 AND p.ipQuoteRequest.id = ?2")
    boolean existsProductById(UUID productId, UUID qrId);

    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN TRUE ELSE FALSE END FROM IpQuoteRequestProductEntity p WHERE p.ipProduct.id = ?1 AND p.ipQuoteRequest.id = ?2 AND p.id <> ?3")
    boolean existsProductById(UUID productId, UUID qrId, UUID qrProductId);

    @Modifying
    @Query("DELETE FROM IpQuoteRequestProductEntity p WHERE p.ipQuoteRequest.id = ?1 AND p.id = ?2")
